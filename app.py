...
def handle_video_note(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🔄 Запросить повторно", callback_data=f"vip_retry_{message.from_user.id}"), # НОВАЯ КНОПКА
...
def mute_user_everywhere(target_id, reason="Без причины", admin_name="Система"):
    # Собираем все чаты сети
    ...
    for cid, name in chats_to_mute.items():
        try:
            # Выдаем бессрочный мут (запрещаем писать сообщения)
    ...
    report_text = (
        ...
        f"• **Кому:** `[{target_id}]`\n"
        f"• **Причина:** {reason}\n"
        f"• **Замучен в группах:** {len(muted_in)} шт."
    )
    try:
...
def handle_vip_decision(call):
    ...
    # Убираем кнопки у админа сразу после нажатия
    bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
