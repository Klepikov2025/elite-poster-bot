...
def mute_user_everywhere(target_id, reason="Без причины", admin_name="Система", user_link=None, trigger_text=None, mute_time=0):
...
def skynet_core_handler(message):
    ...
    try:
        ...
        # Собираем чаты-исключения, где проверка не нужна
        ...
        if chat_id not in EXCLUDED_FROM_PARAMS:
            # Умная маска: понимает Возраст/Рост/Вес и Возраст/Рост/Вес/Размер (в т.ч. 19.5, 19,5, 19*4, 19х4)
            strict_match = re.search(r'(?<!\d)[1-9]\d/1\d{2}/\d{2,3}(?:/\d{1,2}(?:[.,*xхX]\d{1,2})?)?(?!\d)', text)
            
            # Если идеального паттерна НЕТ вообще (любой текст без маски или кривая маска) — КАРАЕМ!
            if not strict_match:
                bot.delete_message(chat_id, message.message_id)
                # Выдаем глобальный мут
                mute_user_everywhere(user_id, reason="Нет параметров или неверный формат (1 Мая)", admin_name="Скайнет 📏", user_link=user_link, trigger_text=trigger_text)
                
                # Отправляем сообщение-инструкцию в чат
                markup = types.InlineKeyboardMarkup()
                ...
                # --- АВТОУДАЛЕНИЕ УВЕДОМЛЕНИЯ ЧЕРЕЗ 5 МИНУТ ---
        ...
        # 5. ПОПРОШАЙКИ (Теперь ГЛОБАЛЬНЫЙ МУТ на 2 часа!)
        if any(word in text for word in YELLOW_BEGGARS):
            bot.delete_message(chat_id, message.message_id)
            mute_user_everywhere(user_id, reason="Желтая зона: Попрошайка", admin_name="Скайнет 🤫", user_link=user_link, trigger_text=trigger_text, mute_time=int(time.time())+7200)
            return
        ...
        if re.search(r'\b18\s*(лет|год|годик|y\.?o\.?)\b|\b18\s*[/\\-]\s*1\d{2}\b|\b(мне|я)\s*18\b', safe_age):
            bot.delete_message(chat_id, message.message_id)
            # Выдаем перманентный глобальный мут
            mute_user_everywhere(user_id, reason="Оранжевая зона: 18 лет", admin_name="Скайнет 🔞", user_link=user_link, trigger_text=trigger_text)
