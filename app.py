...
def process_amnesty_click(call):
    ...
    # 1. Проверяем черный список (скамерам нельзя)
...
def reset_network_stats(call):
    ...
    # 2. Очищаем память о тех, кто стучался, чтобы начать с чистого листа
...
# ================= ОПЛАТА ШТРАФА (АВТОРАЗБАН) =================
    if payload.startswith("fine_payment_"):
        ...
        # Приказ на разбан (через оплату штрафа)
    ...
    try:
        # 1. СНАЧАЛА ВЫДАЕМ СТАТУС В БАЗУ (Чтобы Умный размут увидел права)
        users_collection.update_one({"_id": new_user_id}, {"$set": {"is_vip": True}}, upsert=True)

        # 2. ТЕПЕРЬ ПОЛНАЯ АМНИСТИЯ (Снимаем муты и баны везде)
